...
/*
* Code to find interquartile range
* */
...
//    val input_list= scala.io.StdIn.readLine()
    ...
    val range=IQR(sorted_array,sorted_array.length+1)
    val q3=quartile(sorted_array,sorted_array.length+1)
    if (detect_oulier(9,range,q3)) println("Outlier")
    else println("Not outlier")
  }
  def detect_oulier(n:Int,range:Int,q3:Int):Boolean={

    if (n+range > q3) true
    else false

  }
  def quartile(arr:List[Int],arr_length:Int):Int={
    val mid_index = median(arr, 0, arr_length)

    arr(mid_index + median(arr, mid_index + 1, arr_length))
